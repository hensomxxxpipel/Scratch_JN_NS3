...
class RoutingExperiment
{
  ...
  private:
    ...
    std::string m_CSVfileName{"OLSR_100Node.csv"}; //!< CSV filename.
    int m_nSinks{5};                                      //!< Number of sink nodes.
    std::string m_protocolName{"OLSR"};                    //!< Protocol name.
    double m_txp{12.5};                                     //!< Tx power.
...
RoutingExperiment::Run()
{
    ...
    std::string rate("5632bps");
    std::string phyMode("DsssRate11Mbps");
    std::string tr_name("Percobaan16");
    ...
    addressAdhoc.SetBase("172.16.200.0", "255.255.255.0");
    ...
    CheckThroughput();
        AnimationInterface anim ("OLSR_100Node_Netanim.xml");
    ...
    // packetsReceived = packetsReceived + (iter->second.rxPackets);
    ...
    // NS_LOG_UNCOND("Packet delivery ratio =" << ((packetsReceived*100)/sentPackets)<< "%");
    // NS_LOG_UNCOND("Average Throughput =" << AvgThroughput<< "Kbps");
    // NS_LOG_UNCOND("End to End Delay =" << Delay);
    // NS_LOG_UNCOND("End to End Jitter delay =" << Jitter);
    // NS_LOG_UNCOND("Total Flod id " << j);
    // monitor->SerializeToXmlFile("hasil.xml", true, true);


    if (m_flowMonitor)
    {
        flowmon->SerializeToXmlFile(tr_name + ".flowmon", true, true);
    }
